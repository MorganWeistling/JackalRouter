...
- Fixed: pressing Update in the client failed with "timeout after 30s" while the box was perfectly reachable. `raw.githubusercontent.com` publishes both A and AAAA records; the box has an IPv6 address but no working IPv6 egress, and `socket.create_connection` walks addresses **sequentially** (there is no Happy Eyeballs in the stdlib), so the first AAAA attempt burned the entire `timeout=15` before IPv4 was even tried. Measured on the live box: 15.3 s for a fetch that takes 0.9 s from a laptop — the client's 30 s budget was gone before the update could be applied. GitHub fetches are now pinned to IPv4 (`urlopen_ipv4()`), which is the box's normal mode anyway (IPv6 forwarding is dropped, sing-box runs `ipv4_only`).
